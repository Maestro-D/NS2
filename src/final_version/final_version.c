...
static void chan_close(ssh_session session, ssh_channel channel, void *userdata) {
  ...
  printf("Closing channel to endpoint\n");
}
...
int main() {
  ...
  restart:
  printf("Creating new listener / accepting connection \n");
  ...
  do {
    ssh_event_dopoll(event, 1000);
  } while(!ssh_channel_is_closed(chan) && !ssh_channel_is_closed(client_channel));
